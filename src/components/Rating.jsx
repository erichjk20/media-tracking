  ...
  return (
    <div className={`flex items-center gap-0.5 ${compact ? "mt-1 h-5" : "h-8 gap-1"}`} aria-label={`${value} out of 5 stars`}>
      {[1, 2, 3, 4, 5].map((rating) => {
        ...
        const classes = filled ? "fill-amber-400 text-amber-500" : "text-stone-300 dark:text-stone-700";
        if (readOnly) {
        ...
        return (
          <button
            key={rating}
            className={`inline-flex ${buttonSize} items-center justify-center rounded text-stone-400 transition hover:bg-amber-50 hover:text-amber-500 dark:text-stone-500 dark:hover:bg-amber-950/30 dark:hover:text-amber-300`}
