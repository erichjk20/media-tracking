...
function CoverArt({ item }) {
  return (
    <div className="media-detail-cover relative overflow-hidden rounded bg-stone-200 dark:bg-[#24221f]">
      <MediaCover
  ...
  return (
    <div className="grid min-w-0 grid-cols-[20px_minmax(0,1fr)] gap-2">
      ...
      <div className="min-w-0">
        ...
        <p className="break-words text-xs font-medium leading-4 text-stone-900 dark:text-[#eee9df]">{value}</p>
      </div>
    </div>
  );
}
    ...
    <div className="media-detail-back h-full w-full overflow-hidden bg-[#fbfaf7] p-5 text-stone-950 dark:bg-[#181715] dark:text-stone-100 sm:p-6">
      <div className="flex h-full min-h-0 flex-col">
        <div className="flex flex-wrap items-center gap-1.5">
          ...
          <span className="inline-flex h-6 items-center rounded-full bg-stone-200 px-2 text-[10px] font-semibold text-stone-700 dark:bg-white/10 dark:text-stone-200">
            {statusLabels[item.status] || item.status}
          </span>
          {subtypeLabel && (
            <span className="inline-flex h-6 items-center rounded-full bg-amber-100 px-2 text-[10px] font-semibold text-amber-900 dark:bg-amber-950/40 dark:text-amber-200">
              {subtypeLabel}
            </span>
          )}
        </div>
        ...
        {detailRows.length > 0 && (
          <div className="mt-4 grid shrink-0 grid-cols-2 gap-x-4 gap-y-2 border-t border-stone-300/80 pt-3 dark:border-white/10">
            {detailRows.map((row) => (
...
function MediaDetailOverlay({ item, onClose, onComplete, onDelete, onEdit }) {
  ...
  return (
    <div
      className="fixed inset-0 z-40 overflow-y-auto bg-stone-950/76 px-4 py-5 backdrop-blur-md sm:px-6 sm:py-8"
    ...
    >
      <div className="mx-auto flex min-h-full w-full max-w-6xl items-center justify-center">
        ...
        >
          <button
            className="absolute right-0 top-0 z-10 inline-flex h-10 w-10 items-center justify-center rounded-full bg-white/95 text-stone-700 shadow-lg transition hover:bg-white focus:outline-none focus:ring-4 focus:ring-shelf-accent-deep/35 dark:bg-[#11100e]/95 dark:text-stone-200 dark:hover:bg-[#181715]"
            onClick={onClose}
            type="button"
          ...
          <div className="pt-12">
            <div className="media-flip-scene mx-auto aspect-[2/3] w-full max-w-[340px] sm:max-w-[460px]" data-media-detail-content>
              <div className={`media-flip-object h-full w-full ${isFlipped ? "is-flipped" : ""}`}>
                <div className="media-flip-face media-flip-front">
                  <button
                    className="block h-full w-full text-left focus:outline-none focus:ring-4 focus:ring-shelf-accent-deep/45"
            ...
            <div className="mx-auto mt-4 max-w-[460px] text-center">
              ...
              <div className="flex flex-wrap justify-center gap-2" data-media-detail-content>
                <button
                  className="inline-flex h-10 items-center justify-center gap-2 rounded-full bg-white px-4 text-sm font-semibold text-stone-900 shadow-lg transition hover:bg-stone-100 focus:outline-none focus:ring-4 focus:ring-shelf-accent-deep/35 dark:bg-[#d7cec0] dark:text-[#141210] dark:hover:bg-[#e6ded1]"
                ...
                {canComplete && (
                  <button
                    className="inline-flex h-10 items-center justify-center gap-2 rounded-full bg-shelf-accent-deep px-4 text-sm font-semibold text-white shadow-lg transition hover:bg-shelf-accent focus:outline-none focus:ring-4 focus:ring-shelf-accent-deep/35"
                ...
                <button
                  className="inline-flex h-10 items-center justify-center gap-2 rounded-full bg-[#181715]/90 px-4 text-sm font-semibold text-white shadow-lg ring-1 ring-white/10 transition hover:bg-[#24221f] focus:outline-none focus:ring-4 focus:ring-shelf-accent-deep/35"
                  onClick={() => onEdit(item)}
                  type="button"
                >
                  <Edit3 size={16} />
                ...
                <button
                  className="inline-flex h-10 items-center justify-center gap-2 rounded-full bg-red-700 px-4 text-sm font-semibold text-white shadow-lg transition hover:bg-red-800 focus:outline-none focus:ring-4 focus:ring-red-200 dark:bg-red-700/90 dark:hover:bg-red-600"
