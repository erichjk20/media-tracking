          ...
          if (entry.isAddAction) {
            return (
              <div key={entry.id} className="flex min-h-12 items-center justify-center">
                <button
                  className="inline-flex h-12 w-12 -translate-y-3 items-center justify-center rounded-full border-4 border-[#11100e] bg-shelf-accent-deep text-white shadow-lift transition hover:bg-shelf-accent focus:outline-none focus:ring-4 focus:ring-shelf-accent-deep/35"
                  onClick={onAddItem}
                  type="button"
                  aria-label="Add item"
                  title="Add item"
